...
1. EXPLAIN SELECT * FROM users WHERE age>20:-(doesn't work on small databases have to use SET enable_seqscan=off;)
 
  appdb=# EXPLAIN SELECT * FROM users WHERE age>25;
                               QUERY PLAN
------------------------------------------------------------------------
  Index Scan using idx_age on users  (cost=0.14..12.77 rows=36 width=22)
   Index Cond: (age > 25)
   (2 rows)
